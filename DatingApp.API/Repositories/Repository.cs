...
namespace DatingApp.API.Repositories {
  ...
  public async Task<User> GetUser(int id, bool isCurrentUser)
      ...
      private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
         Func<IEnumerable<int>> getLikeFunc = () => {
           if (likers)
            {
            ...
            {
                 return _context.Likes.Where(u => u.LikerId == id).Select(i => i.LikerId);
            }
        };
        using(var tsk =Task<IEnumerable<int>>.Factory.StartNew(() => getLikeFunc()))
             return await tsk;         
          }
